...
#include "parser/token_kinds.hpp"
...
namespace parser {
  ...
  //
  // We diverge from the tweedledee parser here by NOT using a preprocessor.
  // The main reason is so that we can retain the original source structure
  // (with includes) as much as possible
  class parser {
    preprocessor& pp_lexer_;
    source_manager& source_manager_;
    diagnostic_engine& diagnostic_;
    std::unique_ptr<QASM_program> program_;
  ...
  public:
    parser(preprocessor& pp_lexer, source_manager& source_manager, diagnostic_engine& diagnostic)
      ...
      , program_(std::make_unique<QASM_program>(source_manager, diagnostic))
    {}

    // <mainprogram> = OPENQASM <real> ; <translation_unit>
    std::unique_ptr<QASM_program> parse() {
      parse_header();
      parse_translation_unit(true);
      if (error_) {
        return nullptr;
      } else {
  ...
  private:
    ...
    /*! \brief Parse an individual translation unit */
    // <trans_unit>  = <statement> | <trans_unit> <statement>
    ...
    //               | if ( <id> == <nninteger> ) <qop>
    //               | barrier <anylist> ;
    void parse_translation_unit(bool writeout)
    {
      auto body = new Stmt_list;
      auto location = current_token_.location;
      auto file = source_manager_.containing_file(location)
      ...
      auto trans_unit = Translation_unit::create(location, file, body, writeout);
      program_->add_translation_unit(trans_unit);
